...
### Requirement: Validate tags on modify

The system SHALL validate that tag values are non-empty strings after trimming whitespace.

#### Scenario: Invalid tag value
- **WHEN** user runs modify command with a tag that is empty after trimming (e.g., `--tag "  "`)
